-- Ported forward from github.com/Anadi9/anta (supabase-migration.sql).
-- These two tables ALREADY EXIST in the live Supabase project — this file
-- is the schema of record for the rebuild, not a change to apply blindly.
-- It is written to be idempotent so re-running it against the live project
-- is a no-op rather than an error (the original used bare CREATE TRIGGER,
-- which fails on a second run).
